...
function applyDodge(pt: Point, context: ToolContext) {
    ...
    for (let px = 0; px < drawW; px++) {
      ...
      if (distSq <= radius * radius) {
        ...
        if (factor > 0) {
          ...
          imageData.data[destIdx] = Math.min(255, imageData.data[destIdx] * (1 + factor));
          imageData.data[destIdx + 1] = Math.min(255, imageData.data[destIdx + 1] * (1 + factor));
          imageData.data[destIdx + 2] = Math.min(255, imageData.data[destIdx + 2] * (1 + factor));
        }
      }
    }
  }
...
export const DodgeTool: ToolDef = {
  id: 'dodge',
  name: 'Dodge',
  type: 'retouch',
  cursor: 'crosshair',
  ...
  onPointerMove: (pt, ctx) => {
    ...
    lastPoint = stampAlongPath(lastPoint, pt, ctx, applyDodge);
  },
  onPointerUp: (pt, ctx) => {
    if (!isDrawing) return;
    isDrawing = false;
